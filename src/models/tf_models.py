...
class MaskedActionsMLP(DistributionalQModel, TFModelV2):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name, **kwargs):
        ...
        orig_space = obs_space.original_space['board']
        flat_obs_space = spaces.Box(low=np.min(orig_space.low), high=np.max(orig_space.high),
                                    shape=(np.prod(orig_space.shape),))
